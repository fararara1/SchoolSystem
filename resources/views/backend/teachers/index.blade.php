...
<div class="roles-permissions max-w-7xl mx-auto px-6 py-8">
    <div class="flex items-center justify-between mb-8">
        <h2 class="text-2xl font-extrabold text-gray-800 uppercase">Enseignants</h2>
        <a href="{{ route('teacher.create') }}" class="inline-flex items-center bg-green-600 hover:bg-green-700 text-white text-sm uppercase font-semibold py-2 px-5 rounded shadow transition">
    ...
    <div class="overflow-x-auto bg-white rounded-lg shadow border border-gray-200">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-800 text-white uppercase text-sm font-semibold">
                <tr>
                    <th class="px-6 py-3 text-left w-2/12">Nom</th>
            ...
            <tbody class="divide-y divide-gray-100 text-gray-700 text-sm">
                @foreach ($teachers as $teacher)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 font-semibold">{{ $teacher->user->name }}</td>
                        <td class="px-6 py-4">{{ $teacher->user->email }}</td>
                        <td class="px-6 py-4">{{ $teacher->phone }}</td>
                        <td class="px-6 py-4 text-right space-x-2">
                            <a href="{{ route('teacher.edit',$teacher->id) }}" class="inline-block p-2 text-green-600 hover:bg-green-100 rounded transition" title="Modifier">
                            ...
                            <button data-url="{{ route('teacher.destroy', $teacher->id) }}" class="deletebtn inline-block p-2 bg-red-600 hover:bg-red-700 text-white rounded transition" title="Supprimer">
